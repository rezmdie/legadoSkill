...
def analyze_book_info_page(
    ...
    """
    [START] 智能书籍详情页分析器 - 严格真实版
    
    功能特点：
    - [SECURE] 强制访问真实网页，获取真实HTML源代码
    - [SECURE] 禁止任何Mock或示例HTML
    - [SECURE] 所有选择器在真实HTML上验证
    - [OK] 深度分析HTML结构
    - [OK] 基于知识库智能匹配模式（仅作参考）
    - [OK] 多策略提取（CSS/XPath/正则）
    - [OK] 自动验证规则准确性
    - [OK] 生成推荐规则
    ...
    try:
        # 强制获取真实HTML - 禁止Mock
        print(f"[WEB] 正在访问真实网页: {url}")
        print("[WARNING]  严格模式：必须访问真实网页，禁止Mock！")
        ...
        if response.status_code != 200:
            raise Exception(
                f"[ERROR] 网页访问失败，HTTP状态码: {response.status_code}\n"
                f"请检查URL是否正确：{url}\n"
                f"确保这是一个真实可访问的网页。"
            )
        ...
        if not validate_real_html_required(html):
            raise Exception(
                "[ERROR] 获取的HTML内容无效，可能不是真实网页！\n"
                "请确保URL指向真实的网页。\n"
                "禁止使用Mock或示例HTML。"
            )
        
        print(f"[OK] 成功获取真实HTML，大小: {len(html)} 字符")
        ...
        try:
            ...
            print(f"[SAVE] HTML已永久保存到: {html_storage_path}")
        except:
        ...
        try:
            ...
            for pattern in common_patterns:
                ...
                if result['valid']:
                    audit_summary += f"[OK] {result['description']}: {result['selector']} ({result['match_count']}个)\n"
                else:
                    audit_summary += f"[ERROR] {result['description']}: {result['selector']} (无效)\n"
        except:
...
- **HTML大小**: {len(html)} 字符
- **验证**: [OK] 已验证为真实网页HTML
- **HTML存储**: [OK] 已永久保存到 `{html_storage_path}`
...
[WARNING] **重要**：
- [OK] 只有经过真实HTML审查验证的模式才能使用
- [OK] 知识库内容仅供参考，必须经过审查
- [OK] 使用`audit_knowledge_base`工具进行完整审查
...
#### 第2步：知识库模式匹配（仅供参考）
[WARNING] 注意：知识库内容仅作参考，所有选择器已在真实HTML上验证！
"""
...
[WARNING] **重要提示**:
- [OK] 上述所有选择器都已在真实HTML上验证
- [OK] 基于真实网页HTML源代码分析
- [OK] 禁止使用Mock或示例数据
- [WARNING] 知识库内容仅供参考，最终选择器基于真实HTML
...
1. [OK] 在浏览器中打开URL，确保可以正常访问
2. [OK] 检查URL是否完整且正确
3. [OK] 确保这是一个真实的网页，不是Mock页面
4. [OK] 如果网站需要登录，请提供已登录的URL或Cookie
