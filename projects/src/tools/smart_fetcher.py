...
def smart_fetch_html(
    ...
    """
    [WEB] 智能获取HTML - 支持各种请求方法和编码
    
    用正确的方式获取真实网页内容，支持GET、POST等各种请求方法，自动检测或指定编码
    
    功能特点：
    - [OK] 支持所有HTTP方法（GET, POST, PUT, DELETE等）
    - [OK] 支持自定义headers
    - [OK] 支持Cookies
    - [OK] 支持请求体
    - [OK] 自动重试机制
    - [OK] 超时控制
    - [NEW] 智能编码检测（支持GBK、UTF-8等）
    - [NEW] 支持指定charset参数
    ...
    try:
        ...
        parsed_charset = charset if charset else None
        ...
        # 发送请求
        print(f"[WEB] 正在发送 {method.upper()} 请求: {url}")
        if parsed_charset:
            print(f"[WEB] 使用指定编码: {parsed_charset}")
        else:
            print(f"[WEB] 使用智能编码检测")
...
def get_full_html(
    ...
    try:
        ...
        if not result['success']:
            return f"[ERROR] 请求失败: {result.get('error', 'Unknown error')}"
        
        if result['status_code'] != 200:
            return f"[ERROR] HTTP状态码: {result['status_code']}"
...
[WARNING] **重要**: 这是完整的HTML源代码，用于生成书源规则
"""
        
        return report.strip()
        
    except Exception as e:
        return f"[ERROR] 获取失败: {str(e)}"
