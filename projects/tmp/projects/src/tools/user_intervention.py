...
def manual_intervention_mode(
    ...
    返回:
        人工干预指南和下一步操作
    """
    ctx = runtime.context if runtime else new_context(method="manual_intervention_mode")
...
def inject_custom_js(
    ...
    返回:
        代码验证和集成建议
    """
    ctx = runtime.context if runtime else new_context(method="inject_custom_js")
...
1. 在浏览器中测试你的代码
2. 如果有问题，告诉我错误信息
3. 如果测试通过，使用`test_custom_js`进行验证
4. 测试通过后，将代码集成到书源规则中

需要我帮你测试这段代码吗？使用`test_custom_js`工具。
"""
...
def correct_selector(
    ...
    返回:
        修正记录和验证建议
    """
    ctx = runtime.context if runtime else new_context(method="correct_selector")
...
def manual_define_rule(
    ...
    返回:
        规则定义记录和测试建议
    """
    ctx = runtime.context if runtime else new_context(method="manual_define_rule")
...
def provide_html_sample(
    ...
    返回:
        HTML分析和建议
    """
    ctx = runtime.context if runtime else new_context(method="provide_html_sample")
...
def save_experience(
    ...
    返回:
        保存确认
    """
    ctx = runtime.context if runtime else new_context(method="save_experience")
...
def record_solution(
    ...
    返回:
        记录确认
    """
    ctx = runtime.context if runtime else new_context(method="record_solution")
