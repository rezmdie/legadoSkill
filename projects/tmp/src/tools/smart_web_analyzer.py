...
"""
    ...
    if analysis['search_info']['found']:
        report += f"✅ 找到 {len(analysis['search_info']['forms'])} 个搜索表单\n\n"
    ...
    if analysis['pagination_info']['found']:
        ...
        report += "❌ 未找到分页功能\n\n"
        report += "💡 可能是无限滚动或AJAX加载\n\n"

    report += "### 📋 列表结构分析\n\n"

    if analysis['list_structure']['list_selectors']:
        report += f"✅ 找到 {len(analysis['list_structure']['list_selectors'])} 个可能的列表容器\n\n"
        report += "**推荐选择器**:\n"
    ...
    if security['csrf_token']:
        report += f"⚠️ 检测到CSRF Token: `{security['csrf_token'][:20]}...`\n\n"
...
"""
    ...
    if list_info['recommended']:
        report += f"✅ 推荐列表选择器: `{list_info['recommended']}`\n\n"
    elif list_info['list_selectors']:
        report += f"✅ 找到 {len(list_info['list_selectors'])} 个可能的列表容器\n\n"
