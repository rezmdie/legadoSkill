...
def format_debug_result(result: Dict[str, Any]) -> str:
    ...
    if not result.get("success"):
        return f"❌ 调试失败: {result.get('error', '未知错误')}"

    output = []

    # 基本信息
    output.append(f"✅ 书源名称: {result.get('bookSourceName', '未知')}")
    output.append(f"📝 测试类型: {result.get('testType', '未知')}")
    if result.get('keyword'):
        output.append(f"🔍 关键词: {result.get('keyword')}")
    output.append("")
...
def quick_test(html_content: str, rule: str) -> str:
    ...
    output = []
    output.append(f"✅ 规则: {rule}")
    output.append(f"📝 单个结果: {result['singleResult']}")

    if result["isList"]:
        output.append(f"📋 列表结果 ({len(result['listResult'])} 项):")
