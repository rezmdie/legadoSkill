...
- 建议在实际环境中测试书源，因为模拟环境可能与真实环境有差异
        ...
        return f"❌ 请求超时：网站 {url} 响应时间过长（超过 {timeout} 秒）\n\n💡 建议：\n1. 检查网站是否可访问\n2. 增加超时时间\n3. 检查网络连接"
    except requests.exceptions.SSLError:
        return f"❌ SSL证书错误：网站 {url} 的证书可能过期或无效\n\n💡 建议：\n1. 检查网站URL是否正确\n2. 网站可能需要使用https协议"
    except requests.exceptions.ConnectionError:
        return f"❌ 连接错误：无法连接到网站 {url}\n\n💡 建议：\n1. 检查URL是否正确\n2. 检查网络连接\n3. 网站可能已关闭或需要特殊访问方式"
...
"""
        ...
        for i, elem in enumerate(elements[:10], 1):
            ...
            # 建议的选择器
            if elem_id:
                result += f"✅ **推荐选择器**: `#{elem_id}`\n\n"
            elif elem_class:
                result += f"✅ **推荐选择器**: `.{elem_class.split()[0]}`\n\n"
