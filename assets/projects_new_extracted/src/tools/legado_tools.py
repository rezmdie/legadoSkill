...
def test_legado_rule(
    ...
    try:
        ...
        formatted_result = []
        formatted_result.append(f"[OK] 规则: {rule}")
        formatted_result.append(f"[NOTE] 单个结果: {result['singleResult']}")

        if result["isList"]:
            formatted_result.append(f"[INFO] 列表结果 ({len(result['listResult'])} 项):")
...
def validate_legado_rules(
    ...
    try:
        ...
        if missing_fields:
            return f"[ERROR] 缺少必填字段: {', '.join(missing_fields)}"

        # 验证规则字段
        report = []
        report.append("[OK] 书源基本信息验证通过")
        ...
        for rule_type in rule_types:
            if rule_type in source:
                ...
                if isinstance(rule, dict):
                    report.append(f"[OK] {rule_type} 存在")
                else:
                    report.append(f"[WARNING] {rule_type} 格式不正确，应为对象")
            ...
            try:
                ...
                for name, rule in test_rules:
                    ...
                    if result.get("success"):
                        report.append(f"[OK] {name}规则测试通过: {rule}")
                    else:
                        report.append(f"[WARNING] {name}规则测试失败: {rule}")

            except Exception as e:
                report.append(f"[WARNING] 实际测试失败: {str(e)}")
