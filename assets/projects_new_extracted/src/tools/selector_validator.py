...
def validate_selector_on_real_web(
    ...
    try:
        ...
        print(f"[OK] 成功获取真实HTML，大小: {html_result['size']} 字符")
        
        # 在真实HTML上验证选择器
        print(f"[SEARCH] 正在真实HTML上测试选择器: {selector}")
...
**测试状态**: {'[OK] 有效' if validation_result['valid'] else '[ERROR] 无效'}
...
- **验证结果**: {'[OK] 通过' if validation_result['extracted_count'] == expect_count else '[ERROR] 不匹配'}
"""
...
1. [OK] 检查URL是否正确且可访问
2. [OK] 检查选择器语法是否正确
3. [OK] 在浏览器中打开URL，使用开发者工具测试选择器
4. [OK] 确保这是一个真实的网页，不是Mock页面

**[WARNING] 重要**: 本验证器禁止使用Mock或示例HTML，必须基于真实网页进行验证。
"""
...
**提取状态**: {'[OK] 成功' if extract_result['success'] else '[ERROR] 失败'}
...
1. [OK] 检查URL是否正确且可访问
2. [OK] 检查选择器语法是否正确
3. [OK] 使用验证工具先测试选择器有效性
4. [OK] 确保这是一个真实的网页，不是Mock页面

**[WARNING] 重要**: 本提取器禁止使用Mock或示例HTML，必须基于真实网页进行提取。
"""
