...
def audit_knowledge_base(
    ...
    返回:
        审查报告
    """
    ctx = runtime.context if runtime else new_context(method="audit_knowledge_base")
...
def get_saved_html(
    ...
    返回:
        HTML内容和存储信息
    """
    ctx = runtime.context if runtime else new_context(method="get_saved_html")
...
2. 使用其他分析工具（如`analyze_book_info_page`）会自动保存HTML
3. 确保URL正确
"""
...
def list_saved_htmls(
    ...
    返回:
        HTML列表
    """
    ctx = runtime.context if runtime else new_context(method="list_saved_htmls")
