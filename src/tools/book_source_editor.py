    ...
    try:
        ...
        if not complete_source:
            if action == "create":
                ...
                result = f"✅ 已更新字段: {target_field} = {new_value}"
            
            elif action == "delete":
                if not target_field:
                    return "❌ 请指定要删除的字段（target_field）"
                ...
                for part in field_parts[:-1]:
                    if part not in current:
                        return f"❌ 字段路径不存在: {target_field}"
                    current = current[part]
                
                last_part = field_parts[-1]
                if last_part in current:
                    del current[last_part]
                    result = f"✅ 已删除字段: {target_field}"
                else:
                    return f"❌ 字段不存在: {target_field}"
    ...
    try:
        ...
        if not issues:
            if warnings:
                ...
                result += "### 📊 总体评估\n"
                result += "✅ 书源配置完整，可以进行APP测试！\n"
                result += "✅ 已添加 **[能跑]** 标签\n"
        else:
            result += "### 📊 总体评估\n"
            result += "❌ 书源配置存在错误，请先修复错误项！\n"
