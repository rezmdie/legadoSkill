...
# 编码检测工具（新增 - 智能检测网站编码）
from tools.charset_detector import (
    detect_charset,
    detect_charset_from_html
)

# 工具调用追踪器（新增 - 追踪所有工具调用）
from utils.tool_tracker import track_tool_call, get_tool_tracker

# 智能请求工具（新增 - 增强版HTTP请求）
from utils.smart_request import get_smart_request, fetch_real_html

# 会话管理器（新增 - 管理会话上下文）
from utils.session_manager import get_session_manager

# 系统提示词加载器（新增 - 统一管理系统提示词）
...
# HTML结构分析器（新增 - 智能分析HTML结构）
from utils.html_structure_analyzer import HTMLStructureAnalyzer

# 规则验证器（新增 - 验证和优化规则）
from utils.rule_validator import RuleValidator, RuleOptimizer
...
def build_agent(ctx=None):
    ...
    # 读取系统提示词（支持从文件读取）
    system_prompt = ""
    if "sp_file" in cfg and cfg["sp_file"]:
        # 从文件读取系统提示词
        sp_file_path = os.path.join(workspace_path, cfg["sp_file"])
        try:
            with open(sp_file_path, 'r', encoding='utf-8') as f:
                system_prompt = f.read()
        except Exception as e:
            print(f"警告：无法读取系统提示词文件 {sp_file_path}：{e}")
            # 回退到使用sp字段
    ...
    # 如果sp字段为空且没有sp_file，使用默认提示词
    if not system_prompt:
        system_prompt = "你是一个有帮助的助手。"
    
    # 🔧 初始化知识库（确保知识库在学习之前已加载）
    ...
    # 🔧 添加系统提示词增强（来自新项目的最佳实践）
    system_prompt = get_system_prompt() or system_prompt
